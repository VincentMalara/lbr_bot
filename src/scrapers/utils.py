...
def get_numbers(x):
    if isinstance(x, str):
        ...
        y = ''
        logger.debug(f"error at get_numbers needs a string as input, {type(x)} has been given instead")
        print(f"error at get_numbers needs a string as input, {type(x)} has been given instead")
    return y
    ...
    if isinstance(question, str):
        ...
        if "somme" in question:
            ...
            if "moitié" in question:
                ...
                print(f"error at answer_question, question unknown : {question}")
                logger.debug(f"error at answer_question, question unknown : {question}")
                answer = 0
    else:
        logger.debug(f"error at answer_question, question is not a string, {type(question)} has been given instead")
        print(f"error at answer_question, question is not a string, {type(question)} has been given instead")
        answer = 0
    return answer
...
def check_page(driver, check_phrase, type_):
    ...
    try:
        ...
        if not page_status:
            print(f'error at check_page : {check_phrase} not found in {type_}')
            logger.debug(f'error at check_page : {check_phrase} not found in {type_}')
    except Exception as e:
        print(f'error at check_page : Not connected to {check_phrase} properly, error : {e}')
        logger.debug(f'error at check_page : Not connected to {check_phrase} properly, error : {e}')
        page_status = False
    return page_status
...
def find_rcs_in_page(page_content, rcs):
    ...
    if len(foundRCS) > 0:
        ...
        logger.debug(f"found RCS is empty, expected is : {rcs}")
    return foundRCS
